...
public class WebuserController {
	...
	@PostMapping("/create")
	public ResponseEntity<?> createWebuser (@RequestBody Webuser webuser) throws Exception {
		try {
            ...
            if(webuserRepository.findByEmployeeId(webuser.getEmployeeId()).isPresent()){
				throw new Exception("WebUser with EmployeeId exists");
			}else if(webuserRepository.findByUsername(webuser.getUsername())!=null){
				throw new Exception("WebUser with username exists");
			}
            else if(webuserRepository.findByEmail(webuser.getEmail())!=null){
            	throw new Exception("WebUser with Email exists");
			}
